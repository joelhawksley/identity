...
On each run, processes all unprocessed streams from the last 3 months.
"""
...
def _ytdlp_base_args() -> list[str]:
    """Return common yt-dlp flags (cookies + JS runtime)."""
...
def get_recent_livestreams(channel_url: str, months: int = 3) -> list[tuple[str, str, str]]:
    """
...
def extract_songs(
    ...
    Returns the output path on success, or None if no segments found.
    """
    if not segments:
        print("No music segments found, skipping.")
        return None
...
def process_stream(
    ...
    try:
        ...
        result = extract_songs(raw_video, segments, songs_video)

        if result is None:
            print(f"No music found in {video_id}, skipping.")
            return True
...
def main():
    ...
    parser.add_argument("--months", type=int, default=3, help="How many months back to check (default: 3)")

...
So I set out to build a solution to our problem. First, I constructed a [Magic Mirror](https://magicmirror.builders/) using an off-the-shelf medicine cabinet and LCD display with its frame removed. It displayed the calendar and weather data we needed:

![Magic Mirror prototype](/img/posts/2026-02-17-timeframe/magic-mirror.jpg)
...
Next, I spent about a year experimenting with various jailbroken Kindle devices, eventually landing on a pair with calendar and weather data. I designed wood enclosures and laser-cut them at the [local library makerspace](https://boulderlibrary.org/makerspaces/bldg-61/). The Kindles took a few seconds to refresh and flash the screen to reset the ink pixels, so they only updated every half hour.

![Kindle e-paper display in laser-cut wood enclosure](/img/posts/2026-02-17-timeframe/kindle-enclosure.jpg)

Software-wise, I built a Ruby on Rails app that fetched the necessary data from Google Calendar and Dark Sky. The Kindles woke up on schedule, loading a URL in the app that rendered a PNG using [IMGKit](https://github.com/csquared/IMGKit). The prototype proved e-paper was the right solution: it was unobtrusive regardless of lighting:

![Kindle display in low light](/img/posts/2026-02-17-timeframe/kindle-enclosure-lit.jpg)
...
![OLED tablet compared with epaper display](/img/posts/2026-02-17-timeframe/oled-comparison.jpg)

So it was back to e-paper. I found a system of displays from [Visionect](https://www.visionect.com/), which came in 6"/10"/13"/32" sizes and could update every 10 minutes for 2-3 months on a single charge.

![Visionect display range](/img/posts/2026-02-17-timeframe/visionect-range.jpg)
...
![Visionect display on fridge](/img/posts/2026-02-17-timeframe/visionect-fridge.jpeg)

The Visionect displays required running custom closed-source software, either on their cloud or locally with Docker. I opted for a local installation on the Raspberry Pi already running the Rails backend. I had my best results _pushing_ images to the Visionect displays, using a recurring background job. Every five minutes, it used IMGKit to generate a PNG and send it to the Visionect API. I extracted the reusable logic into the [visionect-ruby](https://github.com/joelhawksley/visionect-ruby) gem. This setup proved to be incredibly reliable, without a single failure for months at a time. 

### First customer pilot

Visiting friends often asked how they could have a similar system in their home. Three years after the initial prototype, I did my first market test with a potential customer. At their request, I experimented with different formats, including cramming an entire month view onto the 13" screen:

![Monthly calendar view on display](/img/posts/2026-02-17-timeframe/monthly.jpg)

The pilot was unsuccessful. The customer didn't see enough value to justify the $1000 price tag (in 2019!) for the 13" device, let alone anything I'd charge for a subscription service. At around the same time, Visionect started charging a $7/mo per-device fee to run their backend software on premises with Docker, after years of it being free to use. I'd have needed to charge $10/month, if not more, for a single screen! 
...
![Redesigning our home](/img/posts/2026-02-17-timeframe/redesigning.jpg)
...
![Boox Mira Pro e-paper display](/img/posts/2026-02-17-timeframe/mira-pro.jpg)

But the most compelling innovation was having the screen update in realtime. I added a clock, the current song playing on our Sonos system (using https://github.com/jishi/node-sonos-http-api) and the next-hour precipitation forecast from Dark Sky:

![Realtime display updates](/img/posts/2026-02-17-timeframe/realtime.jpg)
...
![Bedroom and mudroom displays](/img/posts/2026-02-17-timeframe/bedroom-mudroom.jpg)
...
![Laundry done notification on display](/img/posts/2026-02-17-timeframe/laundry-done.jpg)
...
![Home Assistant dashboard](/img/posts/2026-02-17-timeframe/ha-dashboard.jpg)

The single status indicator removes the need to scan an entire screen. This change in approach is possible because of one key difference: we have separated the _display_ of information about our devices from _control_ of them.

## What's next

I continue to receive significant interest in the project from visitors to our home and remain focused on bringing it to market, but a few key issues remain:
